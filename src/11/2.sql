...
declare
  ...
  l_curr_worry number(38,0);
  l_new_worry  number(38,0);
  l_monkey_row aoc_monkeys%rowtype;
  l_sql varchar2(4000);

  l_tmp_array apex_t_number;
  l_test_number_product pls_integer := 1;
begin
  for rec in (
    select id, starting_items, test_number
      from aoc_monkeys
  )
  loop 
    ...
    l_test_number_product := l_test_number_product * rec.test_number;
  end loop;

  dbms_output.put_line('Test number product: ' || l_test_number_product);
  ...
  loop
    ...
    loop
      ...
      loop
        ...
        l_curr_worry := mod(l_new_worry, l_test_number_product);
